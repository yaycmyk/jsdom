...
function HtmlToDom(core, parser, parsingMode) {
  ...
  if (parser.DefaultHandler || (parser.ParserStream && parser.treeAdapters)) {
    ...
    } else if (parser.ParserStream && parser.treeAdapters) {
      parser.ParseHtml = function (rawHtml) {
        if (parsingMode === 'xml') {
          throw new Error('Can\'t parse XML with parse5, please use htmlparser2 instead.');
        }
        var instance = new parser.ParserStream({ treeAdapter: parser.treeAdapters.htmlparser2, locationInfo: true });
        instance.end(rawHtml);
        return instance.document.children;
      };
    }
    ...
    if (parser.ParserStream && parser.treeAdapters) {
      this.appendHtmlToElement = function (html, element) {
        ...
        var dom = parser.parseFragment(parentElement, html, { treeAdapter: parser.treeAdapters.htmlparser2 });
